...
pub fn handle(app: &mut App, event: Event) -> Result<()> {
    let command = map_key_event(event, &app.settings.keybindings);
    ...
    match command {
        ...
        Command::InsertNewline => {
            let active = app.active_editor;
            app.editors[active].insert_newline(&app.settings.indent, app.settings.tab_size);
        }
        Command::InsertTab => {
            let active = app.active_editor;
        ...
        Command::Copy => {
            let active = app.active_editor;
            if let Some(selected_text) = app.editors[active].copy_selection() {
                let _ = crate::clipboard::set_clipboard(&selected_text);
            }
        },
        Command::Paste => {
            if let Ok(text) = crate::clipboard::get_clipboard() {
                let active = app.active_editor;
    ...
    let should_trigger_autocomplete = matches!(command, Command::InsertChar(_) | Command::DeleteBackwards);
    if should_trigger_autocomplete && !app.settings.disable_autocomplete {
